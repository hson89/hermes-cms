...
- [ ] T020 [US1] Construct Main Split-View Workspace Component in `apps/content-management-engine/src/components/views/DraftingWorkspace.tsx`
...
- [ ] T042 [P] [US5] Write frontend unit tests for the recovery modal states in `apps/content-management-engine/tests/e2e/test_drafting_workspace.spec.ts`
...
- [ ] T045 [US5] Implement server-side dedicated route `POST /api/drafting-sessions/[id]/rollback` in `apps/content-management-engine/src/app/(payload)/api/drafting-sessions/[id]/rollback/route.ts` to restore snapshot state (both `draftData` with Lexical JSON and `mainMedia` reference) directly in the database to avoid client-side JSON/Markdown conversion roundtrips.
