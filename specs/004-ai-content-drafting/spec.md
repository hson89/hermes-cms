...
- Q: When a user opens the drafting interface for a schema that has an expired (inactive) but recoverable `DraftingSession` (within the 24-hour retention window), how should the UI handle the recovery flow? → A: Recovery Prompt Dialog: Block the view with a glassmorphic modal dialog (using extra-diffused shadows, Noto Serif typography, and gold accents per Alexandria token guidelines): "An unsaved draft was found from [timestamp]. Would you like to resume drafting or start fresh?".
...
- Q: What is the primary typography for the Alexandria design system? → A: Noto Serif for headlines/display, Inter for body text, and Public Sans for labels/metadata. (Corrected from Outfit).
...
1. **Given** an expired drafting session within the 24-hour recovery window, **When** the user navigates back to the drafting workspace for that schema, **Then** the system blocks the screen with a glassmorphic modal "Recovery Dialog" (designed with extra-diffused shadows, Noto Serif headlines, and gold accents per Alexandria token guidelines) offering to resume the draft or start fresh.
2. **Given** a session with multiple version snapshots, **When** the user selects a historical version from the top app bar version list, **Then** the structured editor rolls back all fields to the matching historical snapshot state.
...
- **Visual Identity (Alexandria)**: The UI MUST strictly adhere to the Alexandria design system:
  - **Colors**: Primary (`#094cb2`), Tertiary/Gold (`#6d5e00`), Surface-container (`#efedee`), Outline-variant (`#c3c6d5` at 30% for ghost borders).
  - **Typography**: Noto Serif (Headlines), Inter (Body), Public Sans (Labels/UI Small).
  - **Effects**: Glassmorphism (20px blur) for floating menus and top bars, 0.75rem (`xl`) border radius for containers, no hard borders (use ghost borders).
...
- **FR-003**: System MUST support real-time population of structured fields (Title, Slug, Author, Media, Body) via Server-Sent Events (SSE) standard HTTP unidirectional streaming.
  - **Real-Time Visuals**: Fields being actively drafted MUST display a pulsing indicator, a "Drafting..." label, and use a `typing-cursor` animation.
  - **Body Field**: MUST stream as Markdown and be converted to Payload Lexical JSON server-side.
- **FR-004**: System MUST provide a global "Pause" control in the Structured Editor header to halt active AI generation streams.
- **FR-005**: System MUST provide an "AI SUGGESTS" badge for every AI-populated field, using the Alexandria primary color palette and Material Symbols icons for per-field actions (Edit, Accept, Refresh).
- **FR-006**: System MUST feature a "Style & Tone" selector block above the form fields, allowing authors to select from tenant-scoped tones (e.g., Academic, Punchy, Technical) with descriptive tooltips explaining each tone's impact.
- **FR-007**: System MUST provide a floating AI action bar for the Lexical rich-text editor, using a dark glassmorphic style (`bg-inverse-surface/80`), with "Simplify", "Expand", and "Change Tone" actions.
