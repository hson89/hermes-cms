...
- **FR-011**: System MUST enforce a single-user lock per schema/drafting session; other users attempting to start a session for the same schema MUST receive a "Session in progress" notification. The system MUST proactively release the lock immediately upon explicit user exit or page unload, with a 10-minute inactivity timeout serving as a safety fallback. To release the active lock while retaining data for recovery, active sessions MUST transition to an `'expired'` status after 10 minutes of inactivity and be retained in this `'expired'` state for a 24-hour recovery window before they are permanently purged via a background cleanup process. Upon recovery of an expired session (FR-016), the complete list of 10 historical snapshots remains fully preserved, allowing rollback to pre-expired snapshots.
...
- **DraftingSessions**: A Payload CMS collection representing the active state of an AI drafting interaction.
  ...
  - `snapshots`: Array/Blocks field (capped at 10 items) storing history:
    - `timestamp`: DateTime
...
- **AISuggestion**: A structural value object embedded directly within the `DraftingSessions` snapshot array (representing field-level suggestions for refresh/undo actions), rather than a separate database collection.
...
- **AIAuditLogs**: A tenant-scoped Payload CMS collection documenting every AI request.
  ...
  - `estimatedCost`: Number field (required, cost in USD microcents, e.g. `150` microcents = `0.00015` USD)
