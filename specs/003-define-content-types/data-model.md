...
| `originalSchema` | json | No | No | Store of the originally suggested AI schema (if AI-assisted) for diff visualization. |
...
- `AIPromptHistory` references a `Tenant` (logical 1-to-many) and a `User` (logical 1-to-many).

---

### AIPromptHistory (CMS Monolith)
Logs of all natural language prompts used and the corresponding generation outcomes to refine the model over time.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `prompt` | text | Yes | The natural language prompt entered by the user. |
| `generatedSchema` | json | Yes | The schema generated by the AI model in response to the prompt. |
| `tenant` | relationship | Yes | Tenant scope identifier (logical multi-tenancy). |
| `user` | relationship | Yes | User who initiated the prompt. |
| `timestamp` | datetime | Yes | When the prompt was run. |
